    ...
    for block in blocks:
        ...
        for i, line in enumerate(lines[1:5]):
            if i == 0:
                ...
                p = tf.add_paragraph()
            clean = line.replace('**', '').replace('- ', 'â€¢ ').replace('#', '')
            p.text = clean[:120]
...
<body>
    ...
    <div class="footer">
        <p>AI Business Companion â€¢ Professional Intelligence Platform</p>
    </div>
        ...
        async function generate() {{
            ...
            try {{
                ...
                if (outputType === 'text') {{
                    ...
                    showResult('âœ“ Document generated successfully. Click download to save.', true);
                }}
            }} catch (error) {{
                clearInterval(msgInterval);
...
def get_login_page(error=False):
    ...
    error_html = '<div class="alert alert-error"><span>âš ï¸</span> Invalid email or password</div>' if error else ''
